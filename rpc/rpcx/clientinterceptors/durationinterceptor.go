...
func DurationInterceptor(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
	...
	if err != nil {
		xlog2.Zap().Errorf("rpc-client-fail: server: {%s}, req: {%+v}, reply: {%+v}, err: %+v", serverName, req, reply, err)
		return err
	}
	if duration.Milliseconds() > slowThreshold {
		xlog2.Zap().Warnf("rpc-client-slow: server: {%s}, duration: %dms, req: {%+v}, reply: {%+v}", serverName, duration.Milliseconds(), req, reply)
	}
	return nil
}
