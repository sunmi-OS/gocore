...
func TestConnection(t *testing.T) {
	...
	if err1 == nil {
		log.Errorf("It should be an error since the url is not valid")
	}

	_, err2 := NewConnection("http://www.fakedomain.tld", "core0")
	if err2 != nil {
		log.Errorf("It should not be an error since the url is  valid but got '%s'", err2.Error())
	}

	_, err3 := NewConnection("http://www.fakedomain.tld/select/", "core0")
	if err3 != nil {
		log.Errorf("It should not be an error since the url is  valid but got '%s'", err3.Error())
	}
}
...
func TestBytes2JsonWrongJson(t *testing.T) {
	...
	if err == nil {
		log.Errorf("It should a error when parsing non json format")
	}
	if d != nil {
		log.Errorf("It should a error when parsing non json format")
	}
...
func TestBytes2Json(t *testing.T) {
	...
	d, _ := bytes2json(&data)
	if d["t"] != "s" {
		log.Errorf("t should have s as value")
	}

	if d["two"].(float64) != 2 {
		log.Errorf("two should have 2 as value")
	}
...
func TestHasError(t *testing.T) {
	...
	if hasError(data) != true {
		log.Errorf("Should have an error")
	}
	...
	if hasError(data2) != false {
		log.Errorf("Should not has an error")
	}
}
...
func TestSuccessStatus(t *testing.T) {
	...
	if successStatus(data) != false {
		log.Errorf("Status check should give false but got true")
	}
	...
	if successStatus(data3) != true {
		log.Errorf("Status check should give true but got false")
	}
}
