...
import (
	...
	wk "github.com/sunmi-OS/gocore/v2/tools/protoc-gen/cmd/protoc-gen-sm-openapi/generator/wellknown"
...
const (
	infoURL = "https://github.com/sunmi-OS/gocore/v2/tools/protoc-gen/cmd/protoc-gen-openapi"
)
...
func (g *OpenAPIv3Generator) buildOperationV3(
	...
	// description第一行作为Summary，后面当做description
	summary, description, newTagName := splitDescription(description, tagName)
...
func (g *OpenAPIv3Generator) addSchemasForMessagesToDocumentV3(d *v3.Document, messages []*protogen.Message) {
	...
	for _, message := range messages {
		...
		for _, field := range message.Fields {
			...
			if !isSet {
				str := fieldBindingTagValue(field)
		...
		// printErr("definitionProperties ====> %+v", definitionProperties)
		schema := &v3.Schema{
			Type:        "object",
...
func splitDescription(str, tagname string) (summary string, desc string, tag string) {
	tag = tagname
	if str == "" {
		return
	}
