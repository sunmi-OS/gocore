...
import (
	...
	"github.com/sunmi-OS/gocore/v2/tools/protoc-gen/cmd/protoc-gen-sm-openapi/generator"
...
func main() {
	...
	conf := generator.Configuration{
		...
		EnumType:       flags.String("enum_type", "string", `type for enum serialization. Use "string" for string-based serialization`), // integer string
		...
		OutputMode: flags.String("output_mode", "source_relative", `output generation mode. By default, a single openapi.yaml is generated at the out folder. Use "source_relative' to generate a separate '[inputfile].openapi.yaml' next to each '[inputfile].proto'.`),
	}
	...
	opts.Run(func(plugin *protogen.Plugin) error {
		...
		if *conf.OutputMode == "source_relative" {
			for _, file := range plugin.Files {
				...
				outfileName := strings.TrimSuffix(file.Desc.Path(), filepath.Ext(file.Desc.Path())) + ".yaml"
